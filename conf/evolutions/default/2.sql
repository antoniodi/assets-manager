# --- !Ups

create table "USER" (
  "ID"          VARCHAR NOT NULL PRIMARY KEY,
  "USERNAME"    VARCHAR NOT NULL,
  "EMAIL"       VARCHAR NOT NULL,
  "START_DATE"  TIMESTAMP(0) DEFAULT NOW() NOT NULL,
  "END_DATE"    TIMESTAMP(0) DEFAULT NULL
);

INSERT INTO "USER" ("ID", "USERNAME", "EMAIL", "START_DATE", "END_DATE") VALUES ('1', 'luiscocr', 'anthonydicortes@gmail.com', '2021-01-30 12:00:00', NULL);
INSERT INTO "USER" ("ID", "USERNAME", "EMAIL", "START_DATE", "END_DATE") VALUES ('2', 'carlvare', 'carlvare@gmail.com', NOW(), NULL);

CREATE UNIQUE INDEX "UIDX_USER_EMAIL" ON "USER" ("EMAIL");

create table "ASSET" (
  "ID"              VARCHAR NOT NULL PRIMARY KEY,
  "DESCRIPTION"     VARCHAR NOT NULL,
);

create table "LIABILITY" (
  "ID"                  VARCHAR NOT NULL PRIMARY KEY,
  "DESCRIPTION"         VARCHAR NOT NULL,
  "TOTAL_DEBT_CURRENCY" VARCHAR NOT NULL,
  "TOTAL_DEBT_AMOUNT"   NUMERIC(15,2) NOT NULL
);

create table "EXPENSE" (
  "ID"              VARCHAR NOT NULL PRIMARY KEY,
  "DATE"            TIMESTAMP(0) DEFAULT NOW() NOT NULL,
  "CATEGORY"        VARCHAR NOT NULL,
  "DESCRIPTION"     VARCHAR NOT NULL,
  "CURRENCY"        VARCHAR NOT NULL,
  "AMOUNT"          NUMERIC(15,2) NOT NULL,
  "ID_ASSET"        VARCHAR DEFAULT NULL,
  "ID_LIABILITY"    VARCHAR DEFAULT NULL
);

# --- !Downs

DROP INDEX IF EXISTS "UIDX_USER_EMAIL";

DROP TABLE "USER" if EXISTS;

DROP TABLE "ASSET" if EXISTS;

DROP TABLE "LIABILITY" if EXISTS;

DROP TABLE "EXPENSE" if EXISTS;